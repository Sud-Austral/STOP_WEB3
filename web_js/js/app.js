...
const App = {
    ...
    cacheElements() {
        this.elements = {
            ...
            exportBtn2: document.getElementById('btnExportPdf2')
        };
    },
    ...
    bindEvents() {
        ...
        this.elements.exportBtn2?.addEventListener('click', () => this.exportPdfWithCover());
    },
    ...
    /**
     * Export all views to PDF WITH cover page
     */
    async exportPdfWithCover() {
        if (this.state.isExporting) return;

        this.state.isExporting = true;
        const originalView = this.state.currentView;
        const container = this.elements.viewContainer;
        const btn = this.elements.exportBtn2;

        // Update button state
        const originalBtnText = btn.innerHTML;
        btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Portada...';
        btn.disabled = true;
        ...
        try {
            ...
            for (let i = 0; i < this.config.views.length; i++) {
                ...
                // Always add new page (cover is first page)
                pdf.addPage();
